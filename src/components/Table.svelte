<script lang="ts">
  ...
  type RowClickHandler = (item: any) => void;

  export let onRowClick: RowClickHandler | null = null;
  ...
  function fetchDetails(item: any) {
    // Make the API call using the item.id or any other property
    fetch("/prospectLookup", {
      method: "POST",
      ...
      .then((responseData) => {
        console.log("in browser log");
        console.log(responseData);
        // Update the item's details property
        item.details = responseData[0]["email"]; // Assuming the API returns an object with a 'details' property
      })
      .catch((error) => {
  ...
  <div class="tablecontainer">
    {#if maxPages > 1}
      <div class="pagination">
        ...
        <button on:click={nextPage} disabled={currentPage === maxPages - 1}
          >Next</button
        >
      </div>
    {/if}
    ...
    <table>
      ...
      <tbody>
        {#each filteredData.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage) as item, index (index)}
          <tr on:click={() => onRowClick && onRowClick(item)}>
