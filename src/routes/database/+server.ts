...
async function addBreachToDB(scanResult: { Name: any; Title: any; Domain: any; BreachDate: any; AddedDate: any; ModifiedDate: any; PwnCount: any; Description: any; DataClasses: any[]; LogoPath: any; IsVerified: any; IsFabricated: any; IsSensitive: any; IsSpamList: any; IsMalware: any; }[], uid:string|null){
  try{
...
export async function POST({ request }) {
  const { scanResult, uid } = await request.json();
//   console.log(uid)
  const retval = await addBreachToDB(scanResult, uid)
//   return json({retval})
return json('teehee')
